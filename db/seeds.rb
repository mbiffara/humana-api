...
if Rails.env.development? || Hotel.count.zero?
  ...
  hotel1 = Hotel.find_or_create_by!(name: "Shanti Wellness Resort") do |h|
    ...
    h.airport_transfer = "included"
    h.airport_transfer_notes = "Traslado privado incluido en toda reserva de 3 noches o más."
    h.distance_to_center_km = 12.0
    h.pet_friendly = true
    h.pet_dogs = true
    h.pet_cats = false
    h.pet_size_restriction = true
    h.pet_size_restriction_notes = "Hasta 15 kg."
    h.pet_extra_cost = true
    h.pet_extra_cost_notes = "25 EUR por noche."
    h.pet_common_areas = true
    h.pet_specific_rooms = true
    h.group_min_guests = 10
    h.group_max_guests = 40
    h.total_rooms = 24
    h.certified = true
  ...
  hotel2 = Hotel.find_or_create_by!(name: "Casa Cenote Tulum") do |h|
    ...
    h.airport_transfer = "paid"
    h.airport_transfer_notes = "Traslado compartido desde 45 USD por persona."
    h.distance_to_center_km = 7.5
    h.pet_friendly = false
    h.group_min_guests = 20
    h.group_max_guests = 120
    h.total_rooms = 16
    h.certified = true
